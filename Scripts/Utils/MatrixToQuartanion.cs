...
namespace Baku.Quma.Unity
{
    ...
    public static class MatrixToQuaternion
    {
        ...
        {
            float wAbs = 0.5f * Mathf.Sqrt(mat[0, 0] + mat[1, 1] + mat[2, 2] + 1);
            float xAbs = 0.5f * Mathf.Sqrt(mat[0, 0] - mat[1, 1] - mat[2, 2] + 1);
            float yAbs = 0.5f * Mathf.Sqrt(-mat[0, 0] + mat[1, 1] - mat[2, 2] + 1);
            float zAbs = 0.5f * Mathf.Sqrt(-mat[0, 0] - mat[1, 1] + mat[2, 2] + 1);

            //NOTE: Mathf.Sqrtは負値を食うとNaNを吐くはずなので保険
            if (float.IsNaN(wAbs)) wAbs = 0.0f;
            if (float.IsNaN(xAbs)) xAbs = 0.0f;
            if (float.IsNaN(yAbs)) yAbs = 0.0f;
            if (float.IsNaN(zAbs)) zAbs = 0.0f;

            //ホントは必要な3個だけ求めればいいが可読性優先で。
